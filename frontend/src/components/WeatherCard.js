...
import { 
  ...
  ArrowUp, ArrowDown, Activity, Share2 
} from 'lucide-react';
...
const WeatherCard = ({ weather }) => {
  ...
  const { name, main, weather: conditions, wind, visibility, sys } = weather;
    ...
    <div className="w-full max-w-5xl mx-auto mb-8">
      ...
      <div className={`relative glass-strong bg-gradient-to-br ${getBackgroundGradient(condition)} rounded-3xl p-6 sm:p-8 shadow-2xl border border-white/30 overflow-hidden hover-lift`}>
        ...
        <div className="mt-8 flex flex-col sm:flex-row gap-4 justify-center">
          <button className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center justify-center gap-3">
            <Activity size={20} />
            <span>View 7-Day Forecast</span>
          </button>
          
          <button className="px-8 py-4 glass-strong text-slate-700 font-bold rounded-2xl hover:bg-white/40 transition-all duration-300 hover:scale-105 flex items-center justify-center gap-3">
            <MapPin size={20} />
